...
def test_book():
    ...
    for k in range(10):
        new_num_spec_agents = 25
        #Increment by a defined amount of speculators
        if k > 0:
            for l in range(new_num_spec_agents):
        ...
        for i in range(len(agent_list)):
            ...
            if(LOB.get_market_price() != None):
                ...
                (new_num_spec_agents - ) / (new_num_spec_agents)

                speculator_proportion = round((num_one_type_init_agent + k * new_num_spec_agents) / (num_one_type_init_agent*2 + k * new_num_spec_agents),4) * 100
    ...
    fig.update_layout(height=600, width=800, title_text="Market Price and Trend vs. Order Volume", showlegend=False)
    fig.update_xaxes(title_text='Order Volume', row=2, col=1) 
    fig.show()
